...
hooks:
  Stop:
    - hooks:
        - type: command
          command: python "$CLAUDE_PROJECT_DIR/.workflow/scripts/review_driver.py" finish --interface claude --phase fix-tasks --hook-event Stop
---

Review state setup:
!`python "$(git rev-parse --show-toplevel)/.workflow/scripts/review_driver.py" prepare --interface claude --phase fix-tasks --session-id "${CLAUDE_SESSION_ID}" --arguments "$ARGUMENTS"`
